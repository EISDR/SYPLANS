TABLEEVENT = {
    run: function ($scope) {
        ...
        $scope.cell.select = function (event, row) {
            $("tr").removeClass("active text-black");
            $("td").removeClass("active text-black");
            var classElement = "active text-black";
            if (!$(event.currentTarget).parent().hasClass(classElement))
                if (row.selected !== true)
                    if ($scope.activeSET(row) !== false)
                        $(event.currentTarget).parent().addClass("active text-black");
        };
        $scope.cell.dblselect = function (row) {
            $http.get(String.format("{0}/view", $scope.modelName) + `?scope=${$scope.modelName}`, {}).then(function (data) {
        ...
        $scope.deleteRow = async function (row) {
            ...
            BASEAPI.deleteall($scope.tableOrMethod, where, async function (result) {
                if (result.data.error === false) {
                    ...
                    await AUDIT.LOG(AUDIT.ACTIONS.delete, $scope.tableOrView ? $scope.tableOrView : $scope.modelName, row);
        ...
        $scope.importing = function (data) {
            ...
            $scope.procesingRowErrors = [];
            $scope.importedFiles = data.length;
            $scope.importRow();
        };
            ...
            if ($scope.importData.length > 0) {
                ...
                BASEAPI.insertID($scope.tableOrMethod, row.row, '', '', function (result) {
                    ...
                    if (result.data.error === false) {
                        ...
                        $scope.procesingRow++;
                        let new_object = row.row;
                        new_object.error = result.data.error;
                        $scope.invalids.push(new_object)
                        $scope.procesingRowErrors.push(result.data.error);
                        if ($scope.procesingRow === $scope.procesingRowFor || $scope.procesingRowFor === 0) {
                            $scope.procesingRow = 0;
                            $scope.procesingRowFor = 0;
                            $scope.refresh();
                ...
                if ($scope.importData.length === 0) {
                    if ($scope.invalids.length > 0) {
                        $scope.modal.modalView("templates/components/importErrors", {
                            width: 'modal-full',
                            header: {
                                title: `Lista de registros fallidos`,
                                icon: "file-stats"
                            },
                            footer: {
                                cancelButton: true
        ...
        $scope.checkHeaders = function (array, value) {
            return array.some(function (result) {
                return result.column === value;
            });
        }
        $scope.exportErrorXLS = function (id) {
            if (id) {
                var url = $("#" + id).excelexportjs({
                    containerid: id,
                    datatype: 'table',
                    worksheetName: `${$scope.headertitle}.xls`,
                    returnUri: true
                });
                DOWNLOAD.excel("Lista de registros fallidos", url);
            }
        }
        $scope.exportErrorPDF = function (id) {
            $("#" + id).printThis({
