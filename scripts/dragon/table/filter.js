FILTER = {
    ...
    run: function ($scope) {
        ...
        if (myCrud.table.filters !== undefined) {
            ...
            $scope.filters.remove = function (block, index) {
                $scope.filters.blocks.splice(index, 1);
                // =  array.splice(index, 1);
                //
                //     $scope.filters.blocks.filter(function (item) {
                //     return item.id !== block.id;
                // });
            };
            $scope.filters.removeApply = function (index, block, close) {
                $scope.filters.remove(block, index);
                            ...
                            if (item.column.type === FILTER.types.relation) {
                                ...
                                if (itemsElements !== undefined) {
                                    ...
                                    for (const sel of selecteds) {
                                        descriptions.push(eval(`${item.column.text.replaceAll('item', 'sel')}`));
                                    }
                        ...
                        if (Array.isArray(showvalue)) {
                            if (item.column.type === FILTER.types.relation) {
                                ...
                                if (itemsElements != undefined) {
                                    ...
                                    var descriptions = [];
                                    for (const sel of selecteds) {
                                        descriptions.push(`<span class="label bg-${COLOR.secundary} label-rounded">${eval(`${item.column.text.replaceAll('item', 'sel')}`)}</span>`);
                                    }
                                    showvalue = descriptions;
                                }
                            }
                ...
                for (const item of $scope.filters.blocks) {
                    if (!DSON.oseaX(item.column) &&
                        ...
                        if (item.column.type === FILTER.types.relation) {
                            ...
                            if (item.column.type === FILTER.types.integer) {
                                if (item.finalValue == "0" || item.finalValue == "0.00" || item.finalValue == " " || item.finalValue == "") {
                                    whe.value = "0"
                                } else {
                                    whe.value = item.operator.value.replace('{0}', item.finalValue);
                                }
                            } else if (item.column.type === FILTER.types.decimal) {
                                console.log("dios mio", item.finalValue);
                                if (item.finalValue == "0" || item.finalValue == "0.00" || item.finalValue == " " || item.finalValue == "") {
                                    whe.value = "0.00"
                                } else {
                                    whe.value = parseFloat(item.operator.value.replace('{0}', item.finalValue)).toFixed(2);
                                }
                            } else
                                whe.value = item.operator.value.replace('{0}', item.finalValue);

                            console.log('confianza', whe);
                        }
                        where.push(whe);
                    }
                }
