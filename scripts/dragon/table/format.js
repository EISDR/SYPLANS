TABLEFORMAT = {
    run: function ($scope) {
        $scope.cellValueExport = function (key, column, row, originalFormat) {
            ...
            if (column.shorttext) {
                ...
                if (shorttext.length > column.shorttext) {
                    shorttext = String.format(
                        `<a title="${column.title || shorttext.replace(/(<([^>]+)>)/ig, '\r')}">{0}</a>`,
                        shorttext.substring(0, column.shorttext) + "..."
                    );
                } else if (column.showtext) {
                    shorttext = `<a title="${column.title || shorttext.replace(/(<([^>]+)>)/ig, '\r')}">${shorttext}</a>`
                }
                return shorttext;
            }
            return value;
        ...
        $scope.tableStatus = function () {
            ...
            var result = String.format(
                ...
                $scope.plural,
                MESSAGE.i(  `columns.${$scope.table.sortmessage ? $scope.table.sortmessage : $scope.table.orderby}`),
        ...
        $scope.formatDataExport = function (data, extra) {
            var formatedData = [];
            ...
            goData.forEach(function (row) {
                ...
                goColums.forEach(function (column) {
                    ...
                    if (typeof columnObj.export === "function")
                        ...
                        return eval(`newRow.${column} = row.${column};`)
                    } else if (columnObj.export === true)
                        eval(`newRow.${column} = HTML.strip(row.${column});`);
                    else if (typeof columnObj.format === "function")
        ...
        $scope.formatByType = function (column, row, key) {
            ...
            if (column.formattype !== undefined) {
                if (column.formattype.indexOf("datetime") !== -1) {
                    ...
                    var numeralValue = numeral(value)._value;
                    return LAN.money(numeralValue).format(true);
                } else if (column.formattype.indexOf("percentage") !== -1) {
                    if (DSON.oseaX(value)) return DSON.noset();
                    return value + "%";
                } else if (column.formattype.indexOf("password") !== -1) {
