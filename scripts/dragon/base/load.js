LOAD = function () {
    ...
    this.loadContentScope = function (view, id, loadingText, callback, baseDiv, controller, $scope) {
        ...
        var quizas = window.location.href.split('?').length > 1 ? window.location.href.split('?')[1] : "";
        $http.get(view + `?scope=${controller || $scope.modelName}&query=${quizas}`, {}).then(
            function (data) {
                var http = new HTTP();
                http.evaluate(data);
    ...
    this.loadContentView = function (view, $scope, $http, $compile) {
        ...
        var quizas = window.location.href.split('?').length > 1 ? window.location.href.split('?')[1] : "";
        $http.get(view + `?scope=${scope}&query=${quizas}`, {}).then(
            function (data) {
                var http = new HTTP();
                http.evaluate(data);
                if (!http.evaluateTokenHTML(data))
