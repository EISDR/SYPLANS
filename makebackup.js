            ...
            for (const ROW of rows.data) {
                if ((isDay && isHour) || ROW.inmediato == 1) {
                    ...
                    try {
                        ...
                        child = exec(`SET "PGPASSWORD=${conn.password}" && "${source.binpath}\\pg_dump" -h ${conn.host} -p ${conn.port} -U ${conn.user}  -d ${source.database} -F tar --verbose --role "postgres" --format=t --blobs > ${ruta}`);
                    } catch (err) {
