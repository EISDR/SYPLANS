app.controller("vw_auditoria_lista_correctiva", function ($scope, $http, $compile) {
    ...
    if (window.location.href.split('?').length > 0){
        ...
        vw_auditoria_lista_correctiva.evaluacion = window.location.href.split('?')[1] === "evaluate";
    }
    ...
    vw_auditoria_lista_correctiva.headertitle = vw_auditoria_lista_correctiva.evaluacion ? "Seguimiento de Acciones de Mejora" : "Trabajar Acciones de Mejora";
    ...
    } else if (vw_auditoria_lista_correctiva.evaluacion) {
        CRUD_vw_auditoria_lista_correctiva = {};
        DSON.keepmerge(CRUD_vw_auditoria_lista_correctiva, CRUDDEFAULTS);
        DSON.keepmerge(CRUD_vw_auditoria_lista_correctiva, {
            table: {
                ...
                options: [
                    ...
                    },
                    {
                        ...
                        menus: [
                            {
                                text: (data) => {
                                    return "Enviar alerta de Escalamiento";
                                },
                                title: (data) => {
                                    return "Enviar alerta de Escalamiento";
                                },
                                ...
                                click: async function (data) {
                                    ...
                                    let cargos = await BASEAPI.listp('auditoria_lista_correctiva_cargo', {
                                      limit: 0,
                                      where: [
                                          {
                                              field: "auditoria_lista_correctiva",
                                    ...
                                    cargos = cargos.data;
                                    const cargosIds = cargos.map(c => c.cargo);
                                    const responsablesIds = responsables.map(r => r.responsable);
                                    const titulo_push = `Notificación de escalamiento de la acción de mejora: "${data.row.nombre}"`;
                                    const cuerpo_push = `Se ha decidido escalar el trabajo de la acción de mejora para que se puedan tomar las medidas necesarias para solucionarlo. Los encargados y analistas de calidad también estarán recibiendo una copia de esta notificación para mantenerlos al tanto de la situación.

Gracias por su colaboración.`
                                    const titulo_correo = `Notificación de escalamiento de la acción de mejora: "${data.row.nombre}"`;
                                    const cuerpo_correo = `Se ha decidido escalar el trabajo de la acción de mejora para que se puedan tomar las medidas necesarias para solucionarlo. Los encargados y analistas de calidad también estarán recibiendo una copia de esta notificación para mantenerlos al tanto de la situación.
...
Gracias por su colaboración.`
                            ...
                            {
                                text: (data) => {
                                    return "Enviar alerta de plan acción vencido";
                                },
                                title: (data) => {
                                    return "Enviar alerta de plan acción vencido";
                                },
