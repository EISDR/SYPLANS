app.controller("solicitud_documento", function ($scope, $http, $compile) {
    ...
    solicitud_documento.show_diagram = true;
    ...
    solicitud_documento.downloadDiagram = () => {
        let image = solicitud_documento.myDiagram.makeImage({
            scale: 1
        });
        var a = document.createElement("a"); //Create <a>
        a.href = image.src; //Image Base64 Goes here
        a.download = "Flujo de Trabajo - Solicittud de Documentos.png";
        a.click();
    };
    solicitud_documento.diagram = () => {
        solicitud_documento.modal.modalView("solicitud_documento/diagram", {
            width: 'modal-full',
            header: {
            ...
            content: {
                ...
                sameController: 'solicitud_documento',
            },
            event: {
                show: {
                    end: async function (data) {
                        SWEETALERT.loading({message: "Cargando Diagrama"});
                        solicitud_documento.statusList = await BASEAPI.listp('vw_modulo_estatus', {
                            limit: 0,
                            orderby: "entidad",
                            order: "asc",
                            where: [
                                {
                                    "field": "entidad",
                                    "value": "Solicitud de Documentos"
                                }
                            ]
                        });
                        solicitud_documento.statusList = solicitud_documento.statusList.data;
                        setTimeout(async function () {
                            const $$$ = go.GraphObject.make;
                            let statusList = solicitud_documento.statusList;
                            let thejson = {
                            ...
                            solicitud_documento.myDiagram = new go.Diagram(
                            ...
                            solicitud_documento.myDiagram.div.style.backgroundColor = colors.background;
                            solicitud_documento.myDiagram.nodeTemplate = new go.Node('Auto', {
                                isShadowed: true,
                                shadowBlur: 0,
                            ...
                            solicitud_documento.myDiagram.linkTemplate = new go.Link(
                                {
                                    isShadowed: true,
                                    shadowBlur: 0,
                                    shadowColor: 'black',
                            ...
                            solicitud_documento.myDiagram.model = go.Model.fromJson(JSON.stringify(thejson));
                            SWEETALERT.stop();
                        }, 1000)
                    }
                }
