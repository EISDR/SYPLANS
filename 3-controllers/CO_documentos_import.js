app.controller("documentos_import", function ($scope, $http, $compile) {
    ready = async () => {
        ...
        documentos_import.minopli = false;
        ...
        if (documentos_import.session.groupadmin || documentos_import.session.super) {
            documentos_import.fixFilters = [{
                field: "compania",
                value: documentos_import.session.compania_id
            }];
        } else if (documentos_import.session.groups[0]) {
            if (documentos_import.session.groups[0].name) {
                documentos_import.minopli = true;
                documentos_import.fixFilters = [{
                    field: "permisos",
                    operator: 'like',
                    value: `%(${documentos_import.session.groups[0].id})%`
                }, {
                    field: "compania",
                    value: documentos_import.session.compania_id
                }];
        ...
        documentos_import.formulary = function (data, mode, defaultData) {
            ...
            if (documentos_import !== undefined) {
                ...
                documentos_import.createForm(data, mode, defaultData, undefined, () => {
                                ...
                                for (const sheet of entityValue.sheets) {
                                    ...
                                    for (const row of datum) {
                                        ...
                                        for (const field of entityValue.fields[sheet]) {
                                            if (field.tipo === "sheet") {
                                                let find = "";
                                                if (inserteds[field.from])
                                                    if (inserteds[field.from].filter(d => {
                                                        return d;
                                                    }))
                                                        dataToInsert[field.field] = extract || "$null";
                                            } else if (field.tipo === "especial") {
                                                let currentValue = IA.valoresEspeciales[field.from]() || "";
                                                let extract = (currentValue + "").replaceAll("\n", " ").trim();
                                                dataToInsert[field.field] = extract || "$null";
                                            } else if (field.tipo === "SQL") {
        ...
        // });
        //
        documentos_import.triggers.table.after.control = function (data) {
            if (documentos_import.form.options.nombre)
                documentos_import.form.options.nombre.disabled = documentos_import.minopli;
            documentos_import.refreshAngular();
        };
