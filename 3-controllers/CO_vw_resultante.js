    ...
    vw_resultante.getrefresh = async function () {
        ...
        if (vw_resultante.real) {
            vw_resultante.report = await BASEAPI.listp(vw_resultante.soyamfe ? 'vw_resultante_mamfe' : 'vw_resultante', {
                limit: 0,
                orderby: vw_resultante.soyamfe ? "$ compania_base,compania,proceso,elemento,rid,control " : "$ compania_base,compania,factor,table_,departamento,rid,responsable,control ",
                order: "asc",
                where: aymywhere
            });
        } else {
