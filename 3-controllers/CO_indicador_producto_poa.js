app.controller("indicador_producto_poa", function ($scope, $http, $compile) {
    ready = async () => {
        ...
        indicador_producto_poa.rol_bloqueado = await BASEAPI.firstp('vw_presupuesto_aprobado_roles_bloqueados', {
            where: [
                {
                    field: "rol_id",
                    value: indicador_producto_poa.session.groups[0].id
                },
                {
                    field: "departamento",
                    value: indicador_producto_poa.session.departamento
                },
                {
                    field: "poa",
                    value: indicador_producto_poa.session.poa_id
                }
            ]
        });
        ...
        indicador_producto_poa.applyMasksy = function (key, ids) {
            ...
            if (indicador_producto_poa.rol_bloqueado) {
                eval(`indicador_producto_poa.form.options.meta${indicador_producto_poa.list_indicador_producto_poa[key].id}.disabled = true`);
                indicador_producto_poa.list_indicador_producto_poa[key].allow.allow = false;
            }
        };
                ...
                BASEAPI.listp('vw_actualizacion_indicador_poa_periodo', {
                    ...
                    if (indicador_producto_poa.indicador_poa_object != null) {
                        ...
                        if (indicador_producto_poa.form.selected('indicador_poa') !== null) {
                            ...
                            if (indicador_producto_poa.miPOA.periodo_dinamico) {
                                let tienecustom = indicador_producto_poa.form.selected('indicador_poa').poa_monitoreo;
                                if (tienecustom)
                                    indicador_producto_poa.periodicidad = aacontroldemandofalso.periodicidadBy(tienecustom).nombre;
                            }
                            ...
                            if (indicador_producto_poa.list_tipo_meta.length > 0) {
                                ...
                                indicador_producto_poa.tipo_meta_comentario_obligatorio = selected1[0].comentario_obligatorio == 1;
                            }
        ...
        indicador_producto_poa.save_pei_ano = function (indicado, limpi) {
            let indicador = indicador_producto_poa.list_indicador_producto_poa.find(d => d.id === indicado);
            if (indicador_producto_poa.tipo_meta_comentario_obligatorio && indicador.count_comment == 0) {
                SWEETALERT.show({
                    type: 'warning',
