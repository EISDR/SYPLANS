app.controller("solicitud_proceso", function ($scope, $http, $compile) {
    ...
    solicitud_proceso.show_diagram = true;
...
Gracias.`;
    ...
    solicitud_proceso.downloadDiagram = () => {
        let image = solicitud_proceso.myDiagram.makeImage({
            scale: 1
        });
        var a = document.createElement("a"); //Create <a>
        a.href = image.src; //Image Base64 Goes here
        a.download = "Flujo de Trabajo - Solicitud de Procesos.png";
        a.click();
    };
    solicitud_proceso.diagram = () => {
        solicitud_proceso.modal.modalView("solicitud_proceso/diagram", {
            width: 'modal-full',
            header: {
            ...
            content: {
                ...
                sameController: 'solicitud_proceso',
            },
            event: {
                show: {
                    end: async function (data) {
                        SWEETALERT.loading({message: "Cargando Diagrama"});
                        solicitud_proceso.statusList = await BASEAPI.listp('vw_modulo_estatus', {
                            limit: 0,
                            orderby: "entidad",
                            order: "asc",
                            where: [
                                {
                                    "field": "entidad",
                                    "value": "Solicitud de Procesos"
                                }
                            ]
                        });
                        solicitud_proceso.statusList = solicitud_proceso.statusList.data;
                        setTimeout(async function () {
                            const $$$ = go.GraphObject.make;
                            let statusList = solicitud_proceso.statusList;
                            let thejson = {
                            ...
                            solicitud_proceso.myDiagram = new go.Diagram(
                            ...
                            solicitud_proceso.myDiagram.div.style.backgroundColor = colors.background;
                            solicitud_proceso.myDiagram.nodeTemplate = new go.Node('Auto', {
                                isShadowed: true,
                                shadowBlur: 0,
                            ...
                            solicitud_proceso.myDiagram.linkTemplate = new go.Link(
                                {
                                    isShadowed: true,
                                    shadowBlur: 0,
                                    shadowColor: 'black',
                            ...
                            solicitud_proceso.myDiagram.model = go.Model.fromJson(JSON.stringify(thejson));
                            SWEETALERT.stop();
                        }, 1000)
                    }
                }
