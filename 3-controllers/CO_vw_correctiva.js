app.controller("vw_correctiva", function ($scope, $http, $compile) {
    ...
    vw_correctiva.downloadDiagram = () => {
        let image = vw_correctiva.myDiagram.makeImage({
            scale: 1
        });
        var a = document.createElement("a"); //Create <a>
        a.href = image.src; //Image Base64 Goes here
        a.download = "Flujo de Trabajo - Plan de acción.png";
        a.click();
    };
    vw_correctiva.diagram = () => {
        vw_correctiva.modal.modalView("vw_correctiva/diagram", {
            width: 'modal-full',
            header: {
            ...
            content: {
                ...
                sameController: 'vw_correctiva',
            },
            event: {
                show: {
                    end: async function (data) {
                        SWEETALERT.loading({message: "Cargando Diagrama"});
                        vw_correctiva.statusList = await BASEAPI.listp('vw_modulo_estatus', {
                            limit: 0,
                            orderby: "entidad",
                            order: "asc",
                            where: [
                                {
                                    "field": "entidad",
                                    "value": "  Plan de Acción Auditoría  (engloba todas las Acciones de mejoras de una Auditoría)"
                                }
                            ]
                        });
                        vw_correctiva.statusList = vw_correctiva.statusList.data;
                        setTimeout(async function () {
                            const $$$ = go.GraphObject.make;
                            let statusList = vw_correctiva.statusList;
                            let thejson = {
                            ...
                            vw_correctiva.myDiagram = new go.Diagram(
                            ...
                            vw_correctiva.myDiagram.div.style.backgroundColor = colors.background;
                            vw_correctiva.myDiagram.nodeTemplate = new go.Node('Auto', {
                                isShadowed: true,
                                shadowBlur: 0,
                            ...
                            vw_correctiva.myDiagram.linkTemplate = new go.Link(
                                {
                                    isShadowed: true,
                                    shadowBlur: 0,
                                    shadowColor: 'black',
                            ...
                            vw_correctiva.myDiagram.model = go.Model.fromJson(JSON.stringify(thejson));
                            SWEETALERT.stop();
                        }, 1000)
                    }
                }
