app.controller("procesos_mapa_conceptual", function ($scope, $http, $compile) {
    procesos_mapa_conceptual = this;
    //procesos_mapa_conceptual.fixFilters = [];
    //procesos_mapa_conceptual.singular = "singular";
    //procesos_mapa_conceptual.plural = "plural";
    procesos_mapa_conceptual.headertitle = 'Mapas Conceptuales';
    //procesos_mapa_conceptual.destroyForm = false;
    //procesos_mapa_conceptual.permissionTable = "tabletopermission";
    RUNCONTROLLER("procesos_mapa_conceptual", procesos_mapa_conceptual, $scope, $http, $compile);
    procesos_mapa_conceptual.formulary = function (data, mode, defaultData) {
        if (procesos_mapa_conceptual !== undefined) {
            RUN_B("procesos_mapa_conceptual", procesos_mapa_conceptual, $scope, $http, $compile);
            procesos_mapa_conceptual.form.modalWidth = ENUM.modal.width.full;
            procesos_mapa_conceptual.form.titles = {
                new: "Nuevo Mapa Conceptual",
                edit: "Editar Mapa Conceptual",
                view: "Ver Mapa Conceptual"
            };
            procesos_mapa_conceptual.form.readonly = {};
            if (mode == 'new')
                procesos_mapa_conceptual.diagrama = null;
            procesos_mapa_conceptual.createForm(data, mode, defaultData);
            $scope.$watch("procesos_mapa_conceptual.nombre", function (value) {
                var rules = [];
                //rules here
                rules.push(VALIDATION.general.required(value));
                VALIDATION.validate(procesos_mapa_conceptual, 'nombre', rules);
            });
            $scope.$watch("procesos_mapa_conceptual.descripcion", function (value) {
                var rules = [];
                //rules here
                // rules.push(VALIDATION.general.required(value));
                VALIDATION.validate(procesos_mapa_conceptual, 'descripcion', rules);
            });
            //ms_product.selectQueries['tipo_procesos_mapa_conceptual'] = [
            //    {
                //    field: 'id',
                //    operator: '!=',
                //    value: -1
    ...
    procesos_mapa_conceptual.mapa_conceptual = () => {
        ...
        setTimeout(async function () {
            ...
            function relayoutLanes() {
                procesos_mapa_conceptual.myDiagram.nodes.each((lane) => {
                    if (!(lane instanceof go.Group)) return;
                    if (lane.category === 'Pool') return;
                    lane.layout.isValidLayout = false; // force it to be invalid
                });
                procesos_mapa_conceptual.myDiagram.layoutDiagram();
            }
            ...
            procesos_mapa_conceptual.myDiagram = new go.Diagram('myDiagramDiv', {
            ...
            procesos_mapa_conceptual.myDiagram.model = new go.GraphLinksModel({
            ...
            procesos_mapa_conceptual.myDiagram.nodeTemplate = new go.Node('Auto', {
            ...
            procesos_mapa_conceptual.myDiagram.groupTemplateMap.add(
                'Lane',
            ...
            procesos_mapa_conceptual.myDiagram.groupTemplateMap.get('Lane').resizeAdornmentTemplate = new go.Adornment('Spot')
                .add(
                    new go.Placeholder(),
                    new go.Shape({
            ...
            procesos_mapa_conceptual.myDiagram.linkTemplate = new go.Link({
                ...
                doubleClick: function(e, link) {
                    procesos_mapa_conceptual.myDiagram.startTransaction("edit text");
                    const tb = link.findObject("LABEL");
                    if (tb !== null) procesos_mapa_conceptual.myDiagram.commandHandler.editTextBlock(tb);
                    procesos_mapa_conceptual.myDiagram.commitTransaction("edit text");
                }
            }).bind(
            ...
            try {
                procesos_mapa_conceptual.loadMapa(procesos_mapa_conceptual.diagrama);
            } catch (e) {
            ...
            procesos_mapa_conceptual.myDiagram.div.style.backgroundColor = colors.background;

            // Show the diagram's model in JSON format
            function save() {
                procesos_mapa_conceptual.diagrama = procesos_mapa_conceptual.myDiagram.model.toJson();
                procesos_mapa_conceptual.myDiagram.isModified = false;
            }
            function load() {
                procesos_mapa_conceptual.myDiagram.model = go.Model.fromJson(document.getElementById('mySavedModel').value);
                procesos_mapa_conceptual.myDiagram.delayInitialization(relayoutDiagram);
            }
    ...
    procesos_mapa_conceptual.saveMapa = function (){
        procesos_mapa_conceptual.myDiagram.nodes.each(function(node) {
        ...
        procesos_mapa_conceptual.myDiagram.links.each(function(link) {
            if (link.points !== null) {
        ...
        return procesos_mapa_conceptual.myDiagram.model.toJson();
    }

    // Luego, decodificar el texto cuando cargues el diagrama
    procesos_mapa_conceptual.loadMapa = function (json) {
        procesos_mapa_conceptual.myDiagram.model = go.Model.fromJson(json);
    ...
    procesos_mapa_conceptual.addNewPool = function(diagram, poolName) {
    ...
    procesos_mapa_conceptual.addNewLane = function(diagram, laneName) {
...
<p>Por favor, agregar un nuevo grupo y seleccione dando clic sobre el objeto en el diagrama</p>`,
    ...
    procesos_mapa_conceptual.addNode = function (diagram, nodeName, figure) {
    ...
    // });
    //
    procesos_mapa_conceptual.triggers.table.after.open = function (data) {
        procesos_mapa_conceptual.mapa_conceptual();
    ...
    //     return true;
    // };
    procesos_mapa_conceptual.triggers.table.before.insert = (data) => new Promise((resolve, reject) => {
        data.inserting.diagrama = procesos_mapa_conceptual.saveMapa();
        data.inserting.proceso = typeof procesos !== 'undefined' ? procesos?.id : undefined;
    ...
    // };
    procesos_mapa_conceptual.triggers.table.before.update = (data) => new Promise((resolve, reject) => {
        data.updating.diagrama = procesos_mapa_conceptual.saveMapa();
        data.updating.proceso = typeof procesos !== 'undefined' ? procesos?.id : undefined;
