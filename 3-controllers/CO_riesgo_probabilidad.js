app.controller("riesgo_probabilidad", function ($scope, $http, $compile) {
    riesgo_probabilidad = this;
    riesgo_probabilidad.session = new SESSION().current();
    riesgo_probabilidad.entidad = window.location.href.split('?')[1].replaceAll('RF', '');
    riesgo_probabilidad.fixFilters = [
        {
            field: "compania",
            value: riesgo_probabilidad.session.compania_id
        },
        {
            field: "institucion",
            operator: riesgo_probabilidad.session.institucion_id ? "=" : "is",
            value: riesgo_probabilidad.session.institucion_id ? riesgo_probabilidad.session.institucion_id : "$null"
        },
        {
            field: "entidad",
    ...
    if (window.location.href.split('?').length > 2) {
        riesgo_probabilidad.soyamfe = true;
        CRUD_riesgo_probabilidad = {};
        DSON.keepmerge(CRUD_riesgo_probabilidad, CRUDDEFAULTS);
        DSON.keepmerge(CRUD_riesgo_probabilidad, {
            table: {
                //width: "width:3000px;",
                //view: 'vw_riesgo_probabilidad',
                //method: 'riesgo_probabilidad',
                //limits: [10, 50, 100, 0],
                //report: true,
                //batch: false,
                //persist: false,
    ...
    //riesgo_probabilidad.singular = "singular";
    //riesgo_probabilidad.plural = "plural";
    riesgo_probabilidad.headertitle = "Probabilidad";
    if (riesgo_probabilidad.soyamfe)
        riesgo_probabilidad.headertitle = "Ocurrencia";
    //riesgo_probabilidad.destroyForm = false;
    //riesgo_probabilidad.permissionTable = "tabletopermission";
    RUNCONTROLLER("riesgo_probabilidad", riesgo_probabilidad, $scope, $http, $compile);
    riesgo_probabilidad.formulary = function (data, mode, defaultData) {
        if (riesgo_probabilidad !== undefined) {
            RUN_B("riesgo_probabilidad", riesgo_probabilidad, $scope, $http, $compile);
            riesgo_probabilidad.entidad = window.location.href.split('?')[1].replaceAll('RF', '');
            riesgo_probabilidad.form.modalWidth = ENUM.modal.width.full;
            riesgo_probabilidad.form.readonly = {
                compania: riesgo_probabilidad.session.compania_id,
                institucion: riesgo_probabilidad.session.institucion_id,
                entidad: riesgo_probabilidad.entidad
            };
            if (riesgo_probabilidad.soyamfe) {
                riesgo_probabilidad.form.titles = {
                    new: "Nueva Ocurrencia",
                    edit: "Editar Ocurrencia",
                    view: "Ver Ocurrencia"
                };
            }
            if (riesgo_probabilidad.soyamfe) {
                riesgo_probabilidad.form.readonly.mamfe = 1;
            }
            riesgo_probabilidad.createForm(data, mode, defaultData, riesgo_probabilidad.soyamfe ? "form_mamfe" : undefined);
            $scope.$watch("riesgo_probabilidad.nombre", function (value) {
                var rules = [];
                //rules here
                rules.push(VALIDATION.general.required(value));
                VALIDATION.validate(riesgo_probabilidad, 'nombre', rules);
            });
            $scope.$watch("riesgo_probabilidad.descripcion", function (value) {
                var rules = [];
                //rules here
                //rules.push(VALIDATION.general.required(value));
            ...
            //    }
            //];
            $scope.$watch("riesgo_probabilidad.compania", function (value) {
                var rules = [];
                //rules here
                //rules.push(VALIDATION.general.required(value));
                VALIDATION.validate(riesgo_probabilidad, 'compania', rules);
            });
            ...
            //    }
            //];
            $scope.$watch("riesgo_probabilidad.institucion", function (value) {
                var rules = [];
                //rules here
                //rules.push(VALIDATION.general.required(value));
                VALIDATION.validate(riesgo_probabilidad, 'institucion', rules);
            });
        }
    };
    riesgo_probabilidad.triggers.table.before.insert = (data) => new Promise(async (resolve, reject) => {
        //console.log(`$scope.triggers.table.before.insert ${$scope.modelName}`);
        if (riesgo_probabilidad.soyamfe) {
            var exist = await BASEAPI.listp("riesgo_probabilidad", {
                where: [
                    {
                        field: 'compania',
    ...
    riesgo_probabilidad.triggers.table.before.update = (data) => new Promise(async (resolve, reject) => {
        if (riesgo_probabilidad.soyamfe) {
            var exist = await BASEAPI.listp("riesgo_probabilidad", {
                where: [
                    {
