app.controller("login_history", function ($scope, $http, $compile) {
    ...
    login_history.formulary = function (data, mode, defaultData) {
        if (login_history !== undefined) {
            ...
            //    }
            //];
            $scope.$watch("login_history.compania", function (value) {
                var rules = [];
                //rules here
                //rules.push(VALIDATION.general.required(value));
                VALIDATION.validate(login_history, 'compania', rules);
            });
            ...
            //    }
            //];
            $scope.$watch("login_history.institucion", function (value) {
                var rules = [];
                //rules here
                //rules.push(VALIDATION.general.required(value));
                VALIDATION.validate(login_history, 'institucion', rules);
            });
