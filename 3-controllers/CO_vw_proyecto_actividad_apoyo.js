app.controller("vw_proyecto_actividad_apoyo", function ($scope, $http, $compile) {
    ...
    vw_proyecto_actividad_apoyo.show_diagram = true;
    ...
    vw_proyecto_actividad_apoyo.downloadDiagram = () => {
        let image = vw_proyecto_actividad_apoyo.myDiagram.makeImage({
            scale: 1
        });
        var a = document.createElement("a"); //Create <a>
        a.href = image.src; //Image Base64 Goes here
        a.download = "Flujo de Trabajo - Actividades de apoyo (Proyectos Especiales).png";
        a.click();
    };
    vw_proyecto_actividad_apoyo.diagram = () => {
        vw_proyecto_actividad_apoyo.modal.modalView("vw_proyecto_actividad_apoyo/diagram", {
            width: 'modal-full',
            header: {
            ...
            content: {
                ...
                sameController: 'vw_proyecto_actividad_apoyo',
            },
            event: {
                show: {
                    end: async function (data) {
                        SWEETALERT.loading({message: "Cargando Diagrama"});
                        vw_proyecto_actividad_apoyo.statusList = await BASEAPI.listp('vw_modulo_estatus', {
                            limit: 0,
                            orderby: "entidad",
                            order: "asc",
                            where: [
                                {
                                    "field": "entidad",
                                    "value": "Actividades de Apoyo - Proyectos Especiales"
                                }
                            ]
                        });
                        vw_proyecto_actividad_apoyo.statusList = vw_proyecto_actividad_apoyo.statusList.data;
                        setTimeout(async function () {
                            const $$$ = go.GraphObject.make;
                            let statusList = vw_proyecto_actividad_apoyo.statusList;
                            let thejson = {
                            ...
                            vw_proyecto_actividad_apoyo.myDiagram = new go.Diagram(
                            ...
                            vw_proyecto_actividad_apoyo.myDiagram.div.style.backgroundColor = colors.background;
                            vw_proyecto_actividad_apoyo.myDiagram.nodeTemplate = new go.Node('Auto', {
                                isShadowed: true,
                                shadowBlur: 0,
                            ...
                            vw_proyecto_actividad_apoyo.myDiagram.linkTemplate = new go.Link(
                                {
                                    isShadowed: true,
                                    shadowBlur: 0,
                                    shadowColor: 'black',
                            ...
                            vw_proyecto_actividad_apoyo.myDiagram.model = go.Model.fromJson(JSON.stringify(thejson));
                            SWEETALERT.stop();
                        }, 1000)
                    }
                }
