CRUD_presupuesto_aprobado = {};
DSON.keepmerge(CRUD_presupuesto_aprobado, CRUDDEFAULTS);
DSON.keepmerge(CRUD_presupuesto_aprobado, {
    table: {
        engine: 'my',
        ...
        options: [
            {
                ...
                title: (data) => {
                    if (eval(`PERMISSIONS.mypermission.${data.$scope.modelName}`).allow.edit && eval(`PERMISSIONS.mypermission.${data.$scope.modelName}`).allow.view && eval(`PERMISSIONS.mypermission.${data.$scope.modelName}`).allow.remove && eval(`PERMISSIONS.mypermission.${data.$scope.modelName}`).allow.audit){
                        if (data.row.poa_estado != ENUM_2.poa_estatus.Cerrado && data.row.actividades_poa == 'no'){
                        ...
                        if (data.row.poa_estado != ENUM_2.poa_estatus.Cerrado && data.row.actividades_poa == 'no'){
                            return MESSAGE.i('actions.Edit') + ", " +
                                MESSAGE.i('actions.View') + ", " +
                                MESSAGE.i('actions.Remove');
                        } else { return MESSAGE.i('actions.View'); }
                        ...
                        if (data.row.poa_estado != ENUM_2.poa_estatus.Cerrado && data.row.actividades_poa == 'no'){
                            return MESSAGE.i('actions.Edit') + ", " +
                                MESSAGE.i('actions.Remove');
                        } else { return MESSAGE.i('actions.Edit') + ", " +MESSAGE.i('actions.View'); }
                ...
                menus: [
                    {
                        ...
                        show: function (data) {
                            return (data.row.poa_estado != ENUM_2.poa_estatus.Cerrado);
                        },
                        click: function (data) {
                            ...
                            presupuesto_aprobado.id_for_comment = data.row.id;
                            return false;
                        }
                    },
                    {
                    ...
                    },
                    {
                        ...
                        show: (data) =>{
                            return (data.row.poa_estado != ENUM_2.poa_estatus.Cerrado && data.row.actividades_poa == 'no');
                        },
                        click: function (data) {
                            SWEETALERT.confirm({
            ...
            },
            {
                ...
                show: function (data) {
                    return ( data.row.count_comentarios > 0 );
                },
                click: function (data) {
                    presupuesto_aprobado.id_for_comment = data.row.id;
