CRUD_clientes_institucion = {};
DSON.keepmerge(CRUD_clientes_institucion, CRUDDEFAULTS);
DSON.keepmerge(CRUD_clientes_institucion, {
    table: {
        engine: 'my',
        view: "vw_clientes_institucion",
        viewExport: "vw_clientes_institucion_export",
        method: "clientes_institucion",
        sort: "id",
        order: "desc",
        columns: {
            id: {
            ...
            pei: {
                ...
                export: false,
                exportExample: false,
                exportfix: "new SESSION().current().pei_id",
                dead: true
            }
        },
        options: [
            {
                ...
                title: (data) => {
                    if (eval(`PERMISSIONS.mypermission.${data.$scope.modelName}`).allow.edit && eval(`PERMISSIONS.mypermission.${data.$scope.modelName}`).allow.view && eval(`PERMISSIONS.mypermission.${data.$scope.modelName}`).allow.remove && eval(`PERMISSIONS.mypermission.${data.$scope.modelName}`).allow.audit) {
                        if (data.row.estatus_pei != ENUM_2.pei_estatus.Cerrado) {
                ...
                menus: [
                    {
                        ...
                        show: function (data) {
                            return (data.row.estatus_pei != ENUM_2.pei_estatus.Cerrado);
                        },
                        click: function (data) {
