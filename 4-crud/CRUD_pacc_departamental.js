...
lachechon = lachechon || {};
CRUD_pacc_departamental = {};
DSON.keepmerge(CRUD_pacc_departamental, CRUDDEFAULTS);
DSON.keepmerge(CRUD_pacc_departamental, {
    table: {
        //width: "width:3000px;",
        view: 'vw_pacc_departamental',
        method: 'pacc_departamental',
        //limits: [10, 50, 100, 0],
        //report: true,
        batch: false,
        //persist: false,
        ...
        columns: {
            ...
            estatus_details: {
                ...
                format: function(row) {
                    if (row.estatus_details) {
                        var array = eval(row.estatus_details);
                        if (array.includes(1)) {
        ...
        options: [
            ...
            {
                text: (data) => {
                    return "Evaluar PACC Departamental";
                },
                title: (data) => {
                    return "Evaluar PACC Departamental"
                },
                    ...
                    if (typeof vw_pacc != "undefined") {
                        if (vw_pacc) {
                            if (typeof vw_pacc !== 'not defined') {
                                vw_pacc.pacc_dept_id = data.row.id;
                                vw_pacc.show_next_tab = true;
                                vw_pacc.departamento = data.row.departamento_nombre;
                                vw_pacc.codigo_dept = data.row.codigo;
                                vw_pacc.estatus_pacc_departamento = data.row.estatus;
                    ...
                    if (typeof consolidacion != "undefined") {
                        if (consolidacion) {
                            if (typeof consolidacion !== 'not defined') {
                                consolidacion.pacc_dept_id = data.row.id;
                                consolidacion.show_next_tab = true;
                                consolidacion.departamento = data.row.departamento_nombre;
                                consolidacion.codigo_dept = data.row.codigo;
                                consolidacion.estatus_pacc_departamento = data.row.estatus;
                                consolidacion.show_estatus_pacc_dept = data.row.estatus_nombre;
                                consolidacion.cantidad_total_dept = data.row.cantidadtotal;
...
//CRUD_pacc_departamental.table.options[0].menus[0].show = function (data) {
//  return data.row.id > 5;
//};
//add options example, remember add new item in allow object at admin/0-config/security/permission.json
// CRUD_pacc_departamental.table.options[0].menus.push({
//     text: (data) => {
