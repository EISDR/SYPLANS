app.controller("configuration", function ($scope, $http, $compile) {
    ...
    configuration.configurationClose = function () {
        eval(`MODAL.close(${baseController.currentModel.modelName});`);
    };
    ...
    configuration.$scope.$watch('configuration.config.diasdegracia', function (value) {
        var rules = [];
        rules.push(VALIDATION.yariel.numberPositive(value));
        VALIDATION.validate(configuration, "config.diasdegracia", rules);
    });

    configuration.$scope.$watch('configuration.config.horanotificacion', function (value) {
        var rules = [];
        rules.push(VALIDATION.general.required(value));
        VALIDATION.validate(configuration, "config.horanotificacion", rules);
    });
