app.controller("auth", function ($scope, $http, $compile) {
    ...
    if (location.href.indexOf('token=') !== -1) {
        ...
        BASEAPI.firstp('vw_usuario_compania_pei_poa', {
            where: [{
                field: 'confirm',
                value: token
            }]
    ...
    auth.makeLogin = function (tokened) {
        ...
        SERVICE.base_auth.login({
            ...
            if (response.error !== false) {
                ...
                var company = await BASEAPI.firstp('vw_usuario_compania_pei_poa', {
                    where: [
                        {
                            field: "correo",
                            value: auth.username
                ...
                console.log(user, "y la compania", company);
                BASEAPI.mail({
                    "to": admin_email_list,
                ...
                if (response.count[0] > 0) {
                    ...
                    if (user.active == 1) {
                        ...
                        let rsm = await BASEAPI.listp('tipoMeta', {});
                        user.tipoMenta = rsm.data;

                        let rsd = await BASEAPI.listp('direccionMeta', {});
                        user.direccionMeta = rsd.data;

                        let lala = await BASEAPI.listp('vw_aaadef', {limit: 0});
                        user.WORKFLOW = lala.data;

                        rsm = await BASEAPI.listp('poa_monitoreo', {});
                        user.poa_monitorieo = rsm.data;

                        user.poaActual = user.poa_id;
                        let rs = await BASEAPI.listp('drp_poa_navbar', {
                            limit: 0,
                            order: 'asc',
                            orderby: 'periodo_poa',
                            where: [
                        ...
                        if (rs.data)
                            if (rs.data.length > 0) {
                                if (condition) {
                                    user.poaList = rs.data;
                                    user.poaFirt = user.poaList.filter(data => {
                                        return data.id == user.poaActual && data.activo == 1;
                                    });
                                    if (user.poaFirt.length > 0) {
                                        for (var poa in user.poaList) {
                                            if (user.poaFirt[0].id == user.poaList[poa].id) {
                                                user.poaList[poa].selected = true;
                                            } else {
                        ...
                        //         MENU.hideMenus(i);
                        user.GROUPS = GROUPS;
                        // user.mypermission = PERMISSIONS.mypermission;
                        //final
                        ...
                        await BASEAPI.insertp('login_history', {
                            ...
                            institucion: user.institucion_id || "$null",
                            ip: auth.IP,
                        });
                        var http = new HTTP();
                        http.folderredirect('');
    ...
    auth.sendRegistration = async function () {
        ...
        let token = 'SYPLANSTOKEN_' + new Date().getTime();
        ...
        BASEAPI.mail({
            ...
            "fields": { //estos campos se utilizarán en el template
                message: `Usted se ha registrado como la institución/compañía ${auth.nombre}(${auth.sigla}) en el sistema Syplans, se le han adjuntado una serie de documentaciones según su plan adquirido.`,
