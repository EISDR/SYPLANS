FORM = {
    ...
    run: function ($scope, $http) {
            ...
            if (!$scope.yadata) {
                if ($scope.form) {
                    if (!$scope.get_me_once) {
                        ...
                        $scope.form.oldData = $scope.form.getAudit();
                    }
                }
                $scope.yadata = true;
            }
        ...
        $scope.form.getOption2 = function (name, option) {
            if ($scope.form !== null) {
                var property = eval(`$scope.form.options.${name}.${option}`);
                if (!DSON.oseaX(property))
                    return property;
                ...
                if ($scope.form.fileds.indexOf(name) !== -1) {
                    ...
                    if (finalValue !== undefined)
                        if (finalValue != null && finalValue !== undefined && finalValue !== '[NULL]') {
                            ...
                            if (finalValue === '[NULL]')
                                eval(`$scope.form.lastPrepare.${name} = '$null'`);
                        }
                }
            }
        };
            ...
            if ($scope.form.mode === FORM.modes.new) {
                ...
                BASEAPI.insertID($scope.tableOrMethod, $scope.form.inserting, $scope.form.fieldExGET, $scope.form.valueExGET, async function (result) {
                    if (result.data.error === false) {
                        ...
                        var auditVar = $scope.form.getAudit();
            ...
            if ($scope.form.mode === FORM.modes.edit) {
                ...
                BASEAPI.updateall($scope.tableOrMethod, dataToUpdate, async function (result) {
                        if (result.data.error === false) {
                            ...
                            var auditVar = $scope.form.getAudit();
                            $scope.form.oldData.id = DRAGONID;
                            auditVar.id = DRAGONID;
                            if ($scope.auditView)
                                await AUDIT.LOG(AUDIT.ACTIONS.update, $scope.auditView, auditVar, $scope.form.oldData);
                            else

                                await AUDIT.LOG(AUDIT.ACTIONS.update, $scope.tableOrView ? $scope.tableOrView : $scope.modelName, auditVar, $scope.form.oldData);
                            ...
                            if ($scope.form !== null)
                                if ($scope.form.relations !== undefined) {
                                    if (allrelations.length > 0) {
                                        console.log("coñazo", allrelations)
                                        for (var relation of allrelations) {
                                            console.log("coñón", relation)
                                            for (var frel of relation.data) {
                                                console.log("coño", frel)
                                                for (var i in frel) {
                                                    var vi = frel[i].replace('$id', DRAGONID);
                                                    eval(`frel.${i} = vi`);
                                                }
        ...
        $scope.form.loadDropDown = function (name, select2, afterdropdown) {
            var nameclean = name.replace(/\./g, '_');
            if ($scope.form !== null) {
                var animation = new ANIMATION();
                var options = eval(`$scope.form.options.${name}`);
                                ...
                                if (eval(`$scope.form.options.${name}.data`) !== undefined) {
                                    if (eval(`$scope.${name}`) !== "[NULL]") {
                                        ...
                                        if ($scope.brothers[name]) {
                                            $scope.form.options[name].data.push($scope.brothers[name]);
                                        }
                                        var objectSelected = eval(`$scope.form.options.${name}.data`).filter(function (row) {
                                            var idtun = eval(`$scope.form.options.${name}.value`);
                                            idtun = idtun.replaceAll("item.", "row.");
        ...
        $scope.fielsamera = [];
        $scope.form.matchStart = function (params, data) {
        ...
        $scope.form.focusFirstValidation = function (fieldsy) {
            setTimeout(function () {
        ...
        $scope.form.getAudit = function () {
            var obj = {};
            for (var field of $scope.form.fileds) {
                if (field.indexOf('Dragon') == -1) {
                    if ($scope.form.options[field]) {
                        if (!$scope.form.options[field].no_audit) {
                            try {
                                var nameclean = field.replace(/\./g, '_');
                                var descripcion = eval(`$scope.form.options.${field}.placeholder`);
                                if (MESSAGE.exist('columns.' + nameclean)) {
                                    descripcion = MESSAGE.ic('columns.' + nameclean);
                                } else {
                                    if (eval(`$scope.form.options.${field}.placeholder`) === undefined) {
                                        descripcion = eval(`$scope.form.options.${field}.label`);
                                    }
                                }

                                var value = $(`[name='${$scope.modelName}_${nameclean}']`).val() || $(`[name='${$scope.modelName}_${nameclean}']`).html();

                                if ($scope.form.options[field].allowedit !== undefined) {
                                ...
                                } else if ($scope.form.options[field].posttype === "date") {
                                    value = moment($scope[field]).format($scope.form.options[field].format);
                                } else if ($scope.form.options[field].posttype === "checkbox") {
                                    value = $scope[field] ? "Si" : "No";
                                    var descripcion = eval(`$scope.form.options.${field}.text`);
                                }
